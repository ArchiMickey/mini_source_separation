...
def build_2d_time_window_mask(
    ...
    band_ratios: list = [0.175, 0.392, 0.433],
    device="cpu",
    dtype=torch.bool
):
    """
    ...
    Additionally:
    - tokens can attend ONLY to tokens with the same band_group
    """

    left, right = window_size

    # -----------------------
    # 1. Build band groups for each frequency bin
    # -----------------------
    band_ends = [int(F * sum(band_ratios[:i+1])) for i in range(len(band_ratios))]
    band_groups = torch.zeros(F, dtype=torch.long)
    start = 0
    for i, end in enumerate(band_ends):
        band_groups[start:end + 1] = i
        start = end + 1

    # band_groups_flat shape = (N,)
    band_groups_flat = band_groups.repeat(T).to(device)

    # -----------------------
    # 2. Build time indices for each token (shape N)
    # -----------------------
    t_self = torch.arange(T, device=device).repeat_interleave(F)  # (N,)
    t      = t_self.unsqueeze(1)  # (N,1)
    t_kv   = t_self.unsqueeze(0)  # (1,N)

    # -----------------------
    # 3. Time window mask
    # -----------------------
    dt = t_kv - t  # (N, N)   positive = future, negative = past
    ...
    time_mask = past_ok & future_ok  # (N, N)

    # -----------------------
    # 4. Same-band mask
    # -----------------------
    bg_q  = band_groups_flat.unsqueeze(1)  # (N,1)
    bg_kv = band_groups_flat.unsqueeze(0)  # (1,N)
    same_band = (bg_q == bg_kv)            # (N, N)

    # -----------------------
    # 5. Combine both constraints
    # -----------------------
    allowed = time_mask & same_band
...
class BSRoformer(Fourier):
    ...
    def forward(self, audio: Tensor) -> Tensor:
        ...
        # --- 2. Transformer ---
        if self.window_size != (-1, -1) or self.band_ratios is not None:
