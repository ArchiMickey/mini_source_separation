...
class BSRoformer89c3a(nn.Module):
    def __init__(
        self,
        audio_channels=2,
        sample_rate=48000,
    ...
    def _maybe_checkpoint_block(self, block: nn.Module, x: Tensor) -> Tensor:
        if not self.training or not self.use_checkpoint:
            return block(x, rope=self.rope, pos=None)

        return checkpoint(
            lambda inp: block(inp, rope=self.rope, pos=None),
            x,
            use_reentrant=False,
        )
    ...
    def forward(self, audio: Tensor) -> Tensor:
        ...
        for t_block, k_block in zip(self.t_blocks, self.k_blocks):
            ...
            x = self._maybe_checkpoint_block(t_block, x)

            x = rearrange(x, "(b f) t d -> (b t) f d", b=B)
            x = self._maybe_checkpoint_block(k_block, x)
