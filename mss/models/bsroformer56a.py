...
from mss.models.attention import BlockV2 as Block
from mss.models.relative_attention import BlockV2 as RelativeBlock, ALiBi
...
class BSRoformerBlock(nn.Module):
    ...
    def __init__(self, dim, n_heads, axis='tf'):
        ...
        self.axis = axis
        self.time_block = RelativeBlock(dim, n_heads) if 't' in axis else None
        self.freq_block = Block(dim, n_heads) if 'f' in axis else None

    def forward(self, x: Tensor, rope: RoPE, alibi: ALiBi = None) -> Tensor:
        """
        ...
        if self.time_block is not None:
            ...
            x = self.time_block(x, alibi=alibi, pos=None)
...
class BSRoformer(Fourier):
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.pre_blocks = nn.ModuleList([BSRoformerBlock(dim_sp, dim_sp // dim_head, axis='f') for _ in range(n_pre_layers)])
        self.post_blocks = nn.ModuleList([BSRoformerBlock(dim_sp, dim_sp // dim_head, axis='f') for _ in range(n_post_layers)])
        self.blocks = nn.ModuleList([BSRoformerBlock(dim, dim // dim_head, axis='tf') for _ in range(n_layers)])
        self.alibis = nn.ModuleList([ALiBi(dim // dim_head) for _ in range(n_layers)])
    ...
    def forward(self, audio: Tensor) -> Tensor:
        ...
        for block, alibi in zip(self.blocks, self.alibis):
            x = block(x, rope=self.rope, alibi=alibi) 
