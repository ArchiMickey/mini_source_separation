...
class AttentionUpsample(nn.Module):
    ...
    def forward(self, x: Tensor, band_query: Tensor) -> Tensor:
        ...
        def resize_tokens(tokens: Tensor, target_len: int) -> Tensor:
            if tokens.shape[1] == target_len:
                return tokens
            tokens = rearrange(tokens, "bt n c -> bt c n")
            tokens = F.interpolate(
                tokens, size=target_len, mode="linear", align_corners=False
            )
            return rearrange(tokens, "bt c n -> bt n c")
        ...
        if n_registers == freq_bins:
            ...
            register_tokens_interp = resize_tokens(register_tokens, freq_bins)
            mix_features = torch.cat([register_tokens_interp, query_tokens], dim=-1)
            query_gate = torch.sigmoid(self.query_gate_proj(mix_features))
            base_queries = register_tokens_interp + query_gate * (
                query_tokens - register_tokens_interp
            )
            queries = base_queries + band_pos
        ...
        if n_registers == freq_bins:
            ...
            skip_gate = torch.sigmoid(self.skip_gate_proj(mix_features))
            output_tokens = (
                register_tokens_interp
                + skip_gate * (query_tokens - register_tokens_interp)
                + self.attn_gate.tanh() * upsampled
            )
...
class BSRoformer(Fourier):
    r"""128-band BSRoformer69 variant with adaptive freq pooling."""
    ...
    def forward(self, audio: Tensor) -> Tensor:
        ...
        t_down, freq_down = x.shape[2], x.shape[3]
        time_pos_down, _ = self._get_time_axis_positions(b, t_down, freq_down, x.device)
