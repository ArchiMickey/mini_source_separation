...
class ResidualUnit(nn.Module):
    def __init__(self, in_channels, out_channels, dilation, kernel_size=7, use_snake=False, antialias_activation=False):
        super().__init__()
...
def pixel_unshuffle_1d(input_tensor, downscale_factor):
    batch_size, channels, width = input_tensor.size()
    assert width % downscale_factor == 0, "Width must be divisible by downscale_factor"
    output_tensor = input_tensor.view(batch_size, channels, width // downscale_factor, downscale_factor)
    output_tensor = output_tensor.permute(0, 1, 3, 2).contiguous()
    output_tensor = output_tensor.view(batch_size, channels * downscale_factor, width // downscale_factor)
    return output_tensor

class SampleUnshuffleChannelAveragingDownSampleLayer(nn.Module):
    def __init__(
        self,
        in_channels: int,
        out_channels: int,
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        B, C, N = x.shape
        x = x.view(B, self.out_channels, self.group_size, N)
        x = x.mean(dim=2)
        return x

def pixel_shuffle_1d(input_tensor, upscale_factor):
    batch_size, channels, width = input_tensor.size()
    channels //= upscale_factor
    output_tensor = input_tensor.view(batch_size, channels, upscale_factor, width)
    output_tensor = output_tensor.permute(0, 1, 3, 2).contiguous()
    output_tensor = output_tensor.view(batch_size, channels, width * upscale_factor)
    return output_tensor

class ChannelDuplicatingSampleUnshuffleUpSampleLayer(nn.Module):
    def __init__(
        self,
        in_channels: int,
        out_channels: int,
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        x = pixel_shuffle_1d(x, self.factor)
        return x

class EncoderBlock(nn.Module):
    def __init__(self, in_channels, out_channels, stride, kernel_sizes=[7], dilations=[[1, 3, 9]], use_snake=False, antialias_activation=False, downsample_shortcut='none'):
        super().__init__()
        self.layers = []
        for k, ds in zip(kernel_sizes, dilations):
            for d in ds:
                self.layers.append(ResidualUnit(in_channels=in_channels, out_channels=in_channels, dilation=d, kernel_size=k, use_snake=use_snake))
        ...
        if downsample_shortcut == 'none':
            ...
            self.res = WNConv1d(in_channels, out_channels, kernel_size=2*stride, stride=stride, padding=math.ceil(stride/2))
        elif downsample_shortcut == 'averaging':
            self.res = SampleUnshuffleChannelAveragingDownSampleLayer(in_channels, out_channels, stride)
        else:
            raise NotImplementedError(f"Downsample shortcut {downsample_shortcut} not implemented")
...
class DecoderBlock(nn.Module):
    def __init__(self, in_channels, out_channels, stride, kernel_sizes=[7], dilations=[[1, 3, 9]], use_snake=False, antialias_activation=False, use_nearest_upsample=False, upsample_shortcut='none'):
        super().__init__()
        ...
        for k, ds in zip(kernel_sizes, dilations):
            for d in ds:
                self.layers.append(ResidualUnit(in_channels=out_channels, out_channels=out_channels, dilation=d, kernel_size=k, use_snake=use_snake))
        ...
        if upsample_shortcut == 'none':
            ...
            self.res = WNConvTranspose1d(in_channels, out_channels, kernel_size=2*stride, stride=stride, padding=math.ceil(stride/2))
        elif upsample_shortcut == 'duplicating':
            self.res = ChannelDuplicatingSampleUnshuffleUpSampleLayer(in_channels, out_channels, stride)
        else:
            raise NotImplementedError(f"Upsample shortcut {upsample_shortcut} not implemented")
...
class OobleckEncoder(nn.Module):
    def __init__(self, 
        ...
        for i in range(self.depth-1):
            layers += [EncoderBlock(in_channels=c_mults[i]*channels, out_channels=c_mults[i+1]*channels, stride=strides[i], kernel_sizes=kernel_sizes, dilations=dilations, use_snake=use_snake, downsample_shortcut=downsample_shortcut)]
