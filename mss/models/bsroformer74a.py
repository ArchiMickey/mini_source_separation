...
class BSRoformer(Fourier):
    r"""BSRoformer with hierarchical frequency pyramid ablation."""
    ...
    def __init__(
        ...
        **kwargs,
        ...
        n_bands_half = math.ceil(n_bands_full / 2)
        n_bands_quarter = math.ceil(n_bands_half / 2)

        self.freq_pool1 = AttentionPool(
            dim=dim,
            dim_head=dim_head,
            n_bands_full=n_bands_full,
            pool_factor=2,
        )
        self.freq_pool2 = AttentionPool(
            dim=dim,
            dim_head=dim_head,
            n_bands_full=n_bands_half,
            pool_factor=2,
        )
        self.freq_upsample1 = AttentionUpsample(
            dim=dim,
            dim_head=dim_head,
            n_bands_full=n_bands_half,
            n_registers=n_bands_quarter,
        )
        self.freq_upsample2 = AttentionUpsample(
            dim=dim,
            dim_head=dim_head,
            n_bands_full=n_bands_full,
            n_registers=n_bands_half,
        )
        self.mid_fusion = GEGLUFusion(dim, n_bands=n_bands_half)
        ...
        if n_layers != 6:
            raise ValueError("BSRoformer74a expects n_layers=6")
    ...
    def forward(self, audio: Tensor) -> Tensor:
        ...
        x_half = self.freq_pool1(x)
        t_down = x_half.shape[2]
        f_half = x_half.shape[3]
        time_pos_half, _ = self._get_time_axis_positions(b, t_down, f_half, x.device)
        x_half = self._maybe_checkpoint_block(
            self.blocks[0],
            x_half,
            time_rope=self.rope,
            freq_rope=self.freq_rope,
            time_pos=time_pos_half,
        )
        x_half = self._maybe_checkpoint_block(
            self.blocks[1],
            x_half,
        ...
        x_quarter = self.freq_pool2(x_half)
        f_quarter = x_quarter.shape[3]
        time_pos_quarter, _ = self._get_time_axis_positions(
            b, t_down, f_quarter, x.device
        )
        x_quarter = self._maybe_checkpoint_block(
            self.blocks[2],
            x_quarter,
            time_rope=self.rope,
            freq_rope=self.freq_rope,
            time_pos=time_pos_quarter,
        )
        x_quarter = self._maybe_checkpoint_block(
            self.blocks[3],
            x_quarter,
        ...
        x_half_rec = self.freq_upsample1(x_quarter, skip_half)
        x_half_rec = self.mid_fusion(x_half_rec, skip_half)
        x_half_rec = self._maybe_checkpoint_block(
            self.blocks[4],
            x_half_rec,
            time_rope=self.rope,
            freq_rope=self.freq_rope,
        ...
        x_full_rec = self.freq_upsample2(x_half_rec, band_query_full)
        f_full = x_full_rec.shape[3]
        time_pos_full_down, _ = self._get_time_axis_positions(
            b, t_down, f_full, x.device
        )
        x_full_rec = self._maybe_checkpoint_block(
            self.blocks[5],
            x_full_rec,
            time_rope=self.rope,
            freq_rope=self.freq_rope,
            time_pos=time_pos_full_down,
        )
