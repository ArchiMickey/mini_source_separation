...
class TimeResBlock(nn.Module):
    def __init__(self, channels: int, kernel_size: int = 7, expansion_factor: int = 2):
        super().__init__()
...
class TimeConvBlock(nn.Module):
    def __init__(
        ...
        self.blocks = nn.Sequential(
            *(TimeResBlock(channels, kernel_size=kernel_size) for _ in range(depth))
        )
