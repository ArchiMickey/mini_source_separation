...
from mss.models.bandsplit_linear_fixed import BandSplitLinear
from mss.models.fourier import Fourier
from mss.models.rope import RoPE
from mss.models.gabor_transform import GaborTransform
...
class BSRoformer(Fourier):
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.gabor = GaborTransform(
            ...
            oversampling_factors=[oversampling_factor]
        )

        # Band split
        self.bandsplit = BandSplit(
            sr=sample_rate,
            n_fft=n_fft * oversampling_factor,
            n_bands=n_bands,
    ...
    def forward(self, audio: Tensor) -> Tensor:
        ...
        features = [feat[:, :, 0 : -1, :] for feat in features][0]
        x = torch.view_as_real(features)

        # Convert STFT to mel scale
        x = self.bandsplit.transform(x) + self.bandsplit_linear.transform(x)
        ...
        x = self.bandsplit.inverse_transform(x) + self.bandsplit_linear.inverse_transform(x)
        x = torch.view_as_complex(x)
        out = x * features
        out = self.gabor.inverse_transform([x], audio.shape[-1])
