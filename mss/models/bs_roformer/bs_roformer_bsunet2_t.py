...
class UNetModel(nn.Module):
        ...
        for i in range(levels):
            ...
            if i != levels - 1:
                if downsample_type == "Conv":
                    ...
                    self.input_blocks.append(TimestepEmbedSequential(ResidualBlock(DownSample(channels), PixelUnshuffleChannelAveragingDownSampleLayer(channels, channels, 2))))
                else:
                    raise ValueError(f"Invalid downsample type: {downsample_type}")
                input_block_channels.append(channels)

        self.middle_blocks = TimestepEmbedSequential(
            ResBlock(channels, d_time_emb, out_channels=channels),
    ...
    def forward(self, x: torch.Tensor, time_steps: torch.Tensor):
        ...
        for module in self.input_blocks:
            x = checkpoint(module, x, t_emb) if self.checkpoint else module(x, t_emb)
            x_input_block.append(x)
        
        # Middle blocks
        x = checkpoint(self.middle_blocks, x, t_emb) if self.checkpoint else self.middle_blocks(x, t_emb)
...
if __name__ == "__main__":
    ...
    t = torch.rand(1)
    print(model)
    mixture = torch.randn(1, 2, 44100*2)
    output = model(mixture, t)
    print(output.shape)
    # print(output)
    
    from mss.inference.utils import sample_discrete_euler
    fake = sample_discrete_euler(model, mixture, 10)
    print(fake.shape)