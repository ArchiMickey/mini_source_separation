...
class StftToImage(nn.Module):
    ...
    def __init__(self, in_channels: int, sr: float, n_fft: int, mel_bins: int, out_channels: int):
        ...
        for f in range(self.mel_bins):
            ...
            self.band_nets.append(
                nn.Sequential(RMSNorm(in_dim), nn.Linear(in_dim, out_channels))
            )
            self.inv_band_nets.append(
                nn.Sequential(
                    nn.Linear(out_channels, in_dim * 2), nn.Tanh(), nn.GLU(dim=-1)
                )
            )
...
class UNetModel(nn.Module):
    ...
    def __init__(
        ...
        for i in range(levels):
            ...
            for _ in range(depths[i]):
                ...
                if block_types[i] == "resblock":
                    ...
                    layers = [ResBlock(channels, out_channels=channels_list[i])]
                    layers.append(BSTransformer(channels_list[i], n_heads, tf_layers, attn_dropout_p, ff_dropout_p))
                else:
        ...
        self.middle_blocks = nn.Sequential(
            ...
            BSTransformer(channels, n_heads, tf_layers, attn_dropout_p, ff_dropout_p),
            ResBlock(channels, out_channels=channels),
        )
...
class MLP(nn.Module):
    def __init__(self, dim: int, dropout_p=0.) -> None:
        super().__init__()
...
class TransformerBlock(nn.Module):
    def __init__(self, dim: int, n_heads: int, attn_dropout_p=0., ff_dropout_p=0., linear_attn=False):
        ...
        self.att = attn_klass(dim=dim, n_heads=n_heads, dropout_p=attn_dropout_p)
        self.mlp = MLP(dim=dim, dropout_p=ff_dropout_p)
...
class BasicBSTransformerBlock(Module):
    def __init__(self, dim, n_heads, attn_dropout_p=0., ff_dropout_p=0., linear_attn=False):
        super().__init__()
        self.transformer = TransformerBlock(dim, n_heads=n_heads, attn_dropout_p=attn_dropout_p, ff_dropout_p=ff_dropout_p, linear_attn=True)
        self.transformer_t = TransformerBlock(dim, n_heads=n_heads, attn_dropout_p=attn_dropout_p, ff_dropout_p=ff_dropout_p, linear_attn=linear_attn)
        self.transformer_f = TransformerBlock(dim, n_heads=n_heads, attn_dropout_p=attn_dropout_p, ff_dropout_p=ff_dropout_p, linear_attn=linear_attn)
...
class BSTransformer(Module):
    def __init__(self, dim, n_heads, n_layers, attn_dropout_p=0., ff_dropout_p=0.):
        super().__init__()
        self.norm = RMSNorm2d(dim)
        self.proj_in = nn.Conv2d(dim, dim, 1)
        
        self.transformer_blocks = nn.ModuleList([BasicBSTransformerBlock(dim, n_heads, attn_dropout_p=attn_dropout_p, ff_dropout_p=ff_dropout_p) for _ in range(n_layers)])
