...
from mss.models.flex_attention import BlockV2 as FlexBlock
...
class RoformerBlock(nn.Module):
    ...
    def forward(self, x, registers, rope=None):
        # x: (b, d, t, f)
        inp = x
        x = self.down(x)
        ...
        if self.window_size != (-1, -1):
            ...
            if self.attn_mask_cache is not None and self.attn_mask_cache.shape[2:] == (L, L):
                ...
                mask = F.pad(mask, pad=(N_registers, 0, N_registers, 0), value=True)
...
class BSRoformer(Fourier):
    def __init__(
        ...
        **kwargs
        ...
        # RoPE
        self.rope = RoPE(head_dim=dim // n_heads // 2, max_len=rope_len, context_len=n_registers)
        
        # Registers
        self.registers = nn.Parameter(torch.randn(n_registers, dim))
    ...
    def forward(self, audio: Tensor) -> Tensor:
        ...
        # 1.5 Append registers
        registers = rearrange(self.registers, 'n d -> 1 n d').expand(x.shape[0], -1, -1)  # shape: (b, n_registers, d)
        
        # --- 2. Transformer ---
        for stage in self.stages:
            x, registers = stage(x, registers, rope=self.rope)  # shape: (b, t * f, d)
