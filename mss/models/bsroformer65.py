...
# from mss.models2.dsp2.banks import erb_linear_banks
...
class RoPE(nn.Module):
    ...
    def apply_nd(self, x: Tensor, pos: LongTensor) -> Tensor:
        ...
        for i in range(K):
            p = pos[:, :, i]  # (b, l)
            w = self.w[p][:, :, None, :, :]  # (b, l, 1, h/k/2, 2)
            out[i] = self.rotate(x[i], w)

        return rearrange(out, "k b l n h c -> b l n (k h c)")  # (b, l, n, h)


class MultidimRoPE(nn.Module):
    r"""Multi-dimensional Rotary Position Embedding.

    Extends RoPE to support N-dimensional data like spectrograms (time x frequency).
    When shape is provided, applies N-D position encoding with head_dim split across dimensions.
    When shape is None, falls back to standard 1D RoPE with full head_dim.
    """
    ...
    def __init__(
        ...
        self.head_dim = head_dim
        self.n_dims = n_dims
        self.dim_per_axis = head_dim // n_dims if head_dim % n_dims == 0 else head_dim
        self.fallback_rope = RoPE(head_dim=head_dim, max_len=max_len, base=base)
        if head_dim % n_dims == 0:
            self.rope = RoPE(head_dim=self.dim_per_axis, max_len=max_len, base=base)
            self._supports_nd = True
        else:
            self.rope = self.fallback_rope
            self._supports_nd = False

    def forward(self, x: Tensor, shape: tuple[int, ...] | None = None) -> Tensor:
        B, L, N, H = x.shape

        if shape is None or not self._supports_nd:
            return self.fallback_rope(x)
        ...
        pos_indices = []
        # Generate positions for each dimension
        # For shape (t, f): pos_dim0[i] = i // f, pos_dim1[i] = i % f
        for dim_idx in range(len(shape)):
            divisor = 1
            for j in range(dim_idx + 1, len(shape)):
                divisor *= shape[j]
            pos = torch.arange(L, device=x.device) // divisor % shape[dim_idx]
            pos_indices.append(pos)

        pos = torch.stack(pos_indices, dim=1)
        return self.rope.apply_nd(x, pos)
...
class AttentionBlock(nn.Module):
    ...
    def forward(
        ...
        Outputs:
            out: (b, l, d)
        """
        x = x + self.attn(self.norm1(x), rope=rope, pos=pos)
...
class BandGatedRMSNorm(nn.Module):
    ...
    def forward(self, hidden_states: Tensor, gate: Tensor | None = None) -> Tensor:
        ...
        Args:
            hidden_states: (b*f, t, d) - bands folded into batch
            gate: (b*f, t, d) - optional gate tensor

        Returns:
            out: (b*f, t, d)
        """
        BF, T, D = hidden_states.shape
        F = self.n_bands
        B = BF // F
        ...
        # Apply per-band scale: weight shape (f, d) -> (1, f, 1, d)
        hidden_states = hidden_states.view(B, F, T, D)
        hidden_states = hidden_states * self.weight[None, :, None, :]
        hidden_states = hidden_states.view(BF, T, D)
...
class GatedDeltaNet(nn.Module):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        GatedNorm = (
            (lambda d: BandGatedRMSNorm(d, n_bands, eps))
            if n_bands is not None
            else (lambda d: GatedRMSNorm(d, eps))
        )
        self.fallback_norm = GatedNorm(self.head_v_dim)
...
class GatedAttention(nn.Module):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # Always use standard RMSNorm for q_norm and k_norm
        # BandRMSNorm is only applied in AttentionBlock/DeltaNetBlock on the hidden dim
    ...
    def forward(
        ...
        key_states = self.k_norm(self.k_proj(x).view(hidden_shape))
...
class BSRoformerBlock(nn.Module):
    ...
    def forward(
        ...
        Args:
            ...
            freq_pos: (b*t, f, 2) frequency-axis positions with time index retained.
        ...
        if self.time_block is not None:
            ...
            if isinstance(self.time_block, AttentionBlock):
                x = self.time_block(x, rope=rope, pos=time_pos)
            else:
                x = self.time_block(x)
        ...
        if self.freq_block is not None:
            ...
            x = self.freq_block(x, rope=rope, pos=freq_pos)
...
class BSRoformer(nn.Module):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        banks = erb_linear_banks(
            sr=sample_rate, n_bands=256, max_bandwidth=max_bandwidth
        )
        self.sb_filter = SubbandFilter(sr=sample_rate, banks=banks, factor=factor, chunk_size=32)
        ...
        n_bands_full = len(banks)  # pre/post blocks operate at full band resolution
    ...
    def _build_axis_positions(
        ...
        time_idx = torch.arange(time_steps, device=device, dtype=torch.long)
        freq_idx = torch.arange(freq_bins, device=device, dtype=torch.long)

        time_pos = torch.stack(
            (
                time_idx.view(1, time_steps, 1).expand(freq_bins, -1, -1),
                freq_idx.view(freq_bins, 1, 1).expand(-1, time_steps, -1),
            ),
            dim=-1,
        ...
        freq_pos = torch.stack(
            (
                time_idx.view(time_steps, 1, 1).expand(-1, freq_bins, -1),
                freq_idx.view(1, freq_bins, 1).expand(time_steps, -1, -1),
            ),
            dim=-1,
    ...
    def forward(self, audio: Tensor) -> Tensor:
        ...
        if False:
            ...
            print(f"{complex_sp.shape=}, {x.shape=}")
        ...
        x, pad_left, _ = self.pad_tensor(x)
        x = self.proj_in(x)
        B = x.shape[0]

        T, F = x.shape[2], x.shape[3]
        time_pos_full, freq_pos_full = self._build_axis_positions(B, T, F, x.device)

        for block in self.pre_blocks:
            x = block(x, rope=self.rope, time_pos=time_pos_full, freq_pos=freq_pos_full)
        h = x
        x = self.down(x)

        T_down, F_down = x.shape[2], x.shape[3]
        time_pos_down, freq_pos_down = self._build_axis_positions(
            B, T_down, F_down, x.device
        )

        for block in self.blocks:
            x = block(x, rope=self.rope, time_pos=time_pos_down, freq_pos=freq_pos_down)
    ...
    def pad_tensor(self, x: Tensor) -> tuple[Tensor, int, int]:
        """
        ...
        Returns:
            Padded tensor and left/right time padding, e.g., (b, c, t=204, f), 1, 2.
        """
        # Pad symmetrically in time as much as possible, e.g., 201 -> 204 => 1 left, 2 right
        pad_t = -x.shape[2] % self.patch_size[0]
        pad_left = pad_t // 2
        pad_right = pad_t - pad_left
        x = F.pad(x, pad=(0, 0, pad_left, pad_right))
