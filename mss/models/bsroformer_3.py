...
class BSRoformer(Fourier):
    def __init__(
        ...
        **kwargs
        ...
        # RoPE
        self.rope_f = RoPE(head_dim=dim // n_heads, max_len=rope_len)
    ...
    def forward(self, audio: Tensor) -> Tensor:
        ...
        fr = rearrange(self.freq_registers, 'm d -> 1 m d').expand(x.shape[0], -1, -1)
        x, ps = pack([fr, x], 'b * d')
        
        for f_block in self.f_encoder_blocks:
            x = f_block(x, rope=self.rope_f)  # shape: (b*t, f+m, d)
        
        fr, freq_feats = unpack(x, ps, 'b * d')
        x = rearrange(fr, '(b t) m d -> b (t m) d', b=B)
        r = rearrange(self.registers, 'm d -> 1 m d').expand(x.shape[0], -1, -1)
        x, ps = pack([r, x], 'b * d')
        ...
        pos = self.get_pos(T1, self.num_freq_registers, x.device)  # shape: (T*F, 2)
        for block in self.blocks:
            x = block(x, rope=self.rope, pos=pos, attn_mask=attn_mask)  # shape: (b*f, t, d)
        
        _, x = unpack(x, ps, 'b * d')
        x = rearrange(x, 'b (t m) d -> (b t) m d', m=self.num_freq_registers)
    ...
    def get_pos(self, T, F, device) -> Tensor:
        prefix_t = torch.arange(self.num_registers, device=device).view(-1, 1).repeat(1, 2)
        t = torch.arange(T, device=device) + self.num_registers
