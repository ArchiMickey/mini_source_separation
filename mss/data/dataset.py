...
class MSSDataset(torch.utils.data.Dataset):
    ...
    def get_metadata(self):
        ...
        if 'read_metadata_procs' in self.config:
            read_metadata_procs = int(self.config['read_metadata_procs'])
    ...
    def __getitem__(self, index):
        ...
        # if self.config.target_instrument is not None:
        #     index = self.config.instruments.index(self.config.target_instrument)
        ...
        return {
            "mixture": mix,
            **{instr: res[i] for i, instr in enumerate(self.instruments)},
        }, info

def create_dataloader_from_config(dataset_config, batch_size, num_workers):
