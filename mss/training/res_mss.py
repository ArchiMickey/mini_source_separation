...
from mss.inference.utils import sample_discrete_euler
...
class ShiftMSSTrainingWrapper(pl.LightningModule):
    def __init__(
        self,
        model: Fourier,
        lr=None,
    ...
    def training_step(self, batch, batch_idx):
        ...
        t = torch.rand(audios["mixture"].shape[0]).to(self.device)
        alphas, sigmas = 1-t, t
        alphas, sigmas = map(lambda x: x.view(-1, 1, 1), [alphas, sigmas])
        model_input = audios["vocals"] * alphas + audios["mixture"] * sigmas
        
        target = audios["mixture"] - audios["vocals"]

        model_output = self.mss_model(model_input, t)
        
        loss_info.update({"output": model_output, "targets": target})
...
class MSSDemoCallback(pl.Callback):
    ...
    def on_train_batch_end(self, trainer, module, outputs, batch, batch_idx):
        ...
        try:
            ...
            model_output = sample_discrete_euler(model, model_input, self.sample_steps)
...
class MSSValidateCallback(pl.Callback):
    ...
    def separate(self, model, audio, clip_samples, batch_size):
        ...
        while pointer < clips_num:
            ...
            with torch.no_grad():
                model.eval()
                batch_output = sample_discrete_euler(model, batch_clips, self.sample_steps, verbose=False)
