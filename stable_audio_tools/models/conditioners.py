...
class MultiConditioner(nn.Module):
    ...
    def forward(self, batch_metadata: tp.List[tp.Dict[str, tp.Any]], device: tp.Union[torch.device, str]) -> tp.Dict[str, tp.Any]:
        ...
        for key, conditioner in self.conditioners.items():
            ...
            conditioner_output = conditioner(conditioner_inputs, device)
            assert len(conditioner_output) in [2, 3], f"Conditioner output must be a tuple of length 2 or 3, got {len(conditioner_output)}"
            output[key] = conditioner_output[:2]
            if len(conditioner_output) == 3:
                info[key] = conditioner_output[2]
