...
def extract_lora_ups_down(model, target_replace_module=DEFAULT_TARGET_REPLACE):
    ...
    for _m, _n, _child_module in _find_modules(
        ...
        search_class=[LoraInjectedLinear, LoraInjectedConv2d, LoraInjectedConv1d],
    ):
...
def monkeypatch_or_replace_lora_extended(
    ...
    for _module, name, _child_module in _find_modules(
        model,
        target_replace_module,
        search_class=[nn.Linear, LoraInjectedLinear, nn.Conv2d, LoraInjectedConv2d, nn.Conv1d, LoraInjectedConv1d],
    ):
        ...
        elif (_child_module.__class__ == nn.Conv1d) or (
            _child_module.__class__ == LoraInjectedConv1d
        ):
            if len(loras[0].shape) != 3:
                continue
            _source = (
                _child_module.conv
                if isinstance(_child_module, LoraInjectedConv1d)
                else _child_module
            )
...
def monkeypatch_remove_lora(model):
    for _module, name, _child_module in _find_modules(
        model, search_class=[LoraInjectedLinear, LoraInjectedConv2d, LoraInjectedConv1d]
    ):
        if isinstance(_child_module, LoraInjectedLinear):
        ...
        elif isinstance(_child_module, LoraInjectedConv1d):
            _source = _child_module.conv
            weight, bias = _source.weight, _source.bias
