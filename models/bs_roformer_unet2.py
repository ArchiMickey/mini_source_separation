...
class BSRoformerUNet(Fourier):
    def __init__(
        ...
        for _ in range(depths[-1]):
            self.mid.append(nn.ModuleList([
                TransformerBlock(dim=dims[-1], n_heads=dims[-1] // dim_head, rotary_emb=rotary_emb_t),
                TransformerBlock(dim=dims[-1], n_heads=dims[-1] // dim_head, rotary_emb=rotary_emb_f)
            ]))
        ...
        for i, (dim_in, dim_out) in enumerate(reversed(dim_in_out)):
            ...
            for _ in range(depths[-i-1]):
                up.append(nn.ModuleList([
                    nn.Linear(dim_in + dim_out, dim_out),
                    TransformerBlock(dim=dim_out, n_heads=dim_out // dim_head, rotary_emb=rotary_emb_t),
                    nn.Linear(dim_in + dim_out, dim_out),
    ...
    def forward(self, mixture):
        ...
        for stage in self.downs:
            ...
            for transformer_t, transformer_f in blocks:
        ...
        for transformer_t, transformer_f in self.mid:
            x = rearrange(x, 'b d t f -> (b f) t d')
            x = transformer_t(x)
            x = rearrange(x, '(b f) t d -> (b t) f d', b=batch_size)
            x = transformer_f(x)
        ...
        for stage in self.ups:
            ...
            for linear_t, transformer_t, linear_f, transformer_f in blocks:
                x = rearrange(x, 'b d t f -> (b f) t d')
                x = torch.cat([x, hs_t.pop()], dim=-1)
                x = linear_t(x)
                x = transformer_t(x)
                
                x = rearrange(x, '(b f) t d -> (b t) f d', b=batch_size)
                x = torch.cat([x, hs_f.pop()], dim=-1)
                x = linear_f(x)
                x = transformer_f(x)
